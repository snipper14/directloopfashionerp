       ...
       async fetchDepartment() {
        ...
        async filterResults() {
            ...
            const res = await this.getApi("data/stock/searchItem", {
                params: {
                    page,
                    search: this.search_item.length >= 2 ? this.search_item : ""
                }
            });
        ...
        async removeOrder(data, i) {
            ...
            const shouldDelete = await this.deleteDialogue();
            if (shouldDelete) {
                 var obj=Object.assign(data,{'department_id':this.form_data.department_id})
                const res = await this.callApi("post", "data/quote/deleteOrderItem", data);
        ...
        async addCart(value) {
              ...
              this.form_data.stock_id=value.id
                ...
                this.form_data.row_total=(value.cost_price*value.qty)
            ...
            this.showLoader();
              this.form_data.order_total=parseFloat(this.grand_total)+parseFloat(this.form_data.row_total)
               
              
            const res = await this.callApi(
                "post",
                "data/quote/updateOrder",
                this.form_data
            );
            this.hideLoader();
            if (res.status === 200) {
        ...
        async orders() {
            const res = await this.callApi("post", "data/quote/fetchCompleteQuoteByQuoteNo", {
                customer_id: this.data.customer_id,
                order_no: this.data.order_no
            });
            if (res.status == 200) {
