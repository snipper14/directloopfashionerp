    ...
    <div>
        <fieldset class="border">
            <legend class="text-center">
                Assembly Product ManufacturingReport
            </legend>
                        ...
                        >
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">Raw Material Issued </th>

                                    <th scope="col">Raw Material Deficit</th>
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td><b>{{cashFormatter(data.final_qty_produced)}}</b></td>
                                     <td>
                                        {{cashFormatter( data.total_ingredient_qty_issued )}}
                                    </td>
                                    <td>
                                        {{
                                           cashFormatter( data.total_ingredient_pending)
                                        }}
                                    </td>
                        ...
                        <div class="row">
                            <div class="col-4 d-flex" v-if="isDownloadPermitted">
                                <export-excel
                                    ...
                                    :fetch="fetchExcelManufacturedItems"
...
export default {
    ...
    methods: {
        async fetchExcelManufacturedItems(){
            this.showLoader()
             const res = await this.getApi("data/dispatch_assembly_req/fetchManufactureProducts", 
             {
                  params: {
            ...
            for (var i = 0; i < manufacture_product_array.length; i++) {
                      ...
                      manufacture_product_array[i].stock = manufacture_product_array[i].stock
                    ? manufacture_product_array[i].stock.product_name
                    : "";
                  
                manufacture_product_array[i].branch = manufacture_product_array[i].branch
                    ? manufacture_product_array[i].branch.branch
                    : "";
                manufacture_product_array[i].user = manufacture_product_array[i].user
                    ? manufacture_product_array[i].user.name
                    : "";
                     manufacture_product_array[i].department = manufacture_product_array[i].department
        ...
        async fetchReport(page) {
            ...
            const res = await this.getApi(
                ...
                "data/dispatch_assembly_req/fetchManufactureProducts",
                {
                    params: {
                        page,
