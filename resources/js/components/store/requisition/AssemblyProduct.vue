...
export default {
    ...
    methods: {
        async fetchRawMaterials(data) {
            add_material_modal = true;
            const res = await this.getApi(
                "data/assembly_requisition/fetchRawMaterials",
                {
                    params: {
                        req_id: data.req_id,
        ...
        async downloadReq() {
            ...
            axios({
                ...
                this.$router.push("assembly_product_requisition_dashboard");
            });
        },
        ...
        async fetchPendingReq() {
            this.showLoader();
            const res = await this.getApi(
                "data/assembly_requisition/fetchPendngReq"
            );
            this.hideLoader();
            if (res.status === 200) {
        ...
        async deleteData(id, i) {
            ...
            if (shouldDelete) {
                const res = await this.callApi(
                    "delete",
                    "data/assembly_requisition/deleteItem/" + id,
                    ""
                );
        ...
        async submitRecords(data) {
            ...
            const res = await this.callApi(
                "post",
                "data/assembly_requisition/create",
                this.form_data
            );
            this.hideLoader();
            if (res.status == "200") {
                ...
                if (res.data.length > 0) {
                    ...
                    this.swr("This product has no raw materials");
                }
            } else {
                if (res.status == 422) {
